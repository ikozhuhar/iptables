...
| `-i (--in-interface)` | Входящий интерфейс (только для INPUT, FORWARD, PREROUTING) |
| `-o (--out-interface)` | Исходящий интерфейс (только для OUTPUT, FORWARD, POSTROUTING) |
| `-s (--source)` | IP-адрес источника (хост или подсеть, например 192.168.1.0/24) |
| `-d (--destination)` | IP-адрес назначения |
| `--dport` | Порт назначения (требует -p tcp или -p udp) |
| `--sport` | Порт источника (требует -p tcp или -p udp) |
